...
serve(async (req) => {
  ...
  try {
    ...
    // Generate stunning hero image using Lovable AI (Nano banana)
    console.log('Generating high-quality hero image with Lovable AI...');
    let imageUrl = null;
    let imageCredit = 'AI Generated by Cardinal News (Lovable AI)';
...
Generate an image that readers will immediately recognize as related to this specific news story.`;
        ...
        if (imageResponse.ok) {
          ...
          if (generatedImageUrl) {
            ...
            console.log('No image data received from Lovable AI');
          }
        } else {
          console.error('Lovable AI image generation failed:', imageResponse.status, await imageResponse.text());
        }
      }
    } catch (imageError) {
