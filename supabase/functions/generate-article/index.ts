...
serve(async (req) => {
  ...
  try {
    ...
    // Generate stunning hero image using OpenAI's powerful gpt-image-1
    console.log('Generating high-quality hero image with OpenAI gpt-image-1...');
    let imageUrl = null;
    let imageCredit = 'AI Generated by Cardinal News (OpenAI GPT-Image-1)';

    try {
      const OPENAI_API_KEY = Deno.env.get('OPENAI_API_KEY');
      if (!OPENAI_API_KEY) {
        console.warn('OPENAI_API_KEY not set, using fallback image');
      } else {
        ...
        if (imageResponse.ok) {
          ...
          if (generatedImageBase64) {
            // Base64 is already in the right format from OpenAI
          ...
          const filename = `articles/${sanitizedTitle}-${timestamp}.png`;
          ...
          const { error: uploadError } = await supabaseClient.storage
            ...
            if (!uploadError) {
              ...
              imageUrl = publicUrl;
              console.log('✓ High-quality hero image uploaded successfully:', publicUrl);
            } else {
          ...
          console.error('OpenAI image generation failed:', imageResponse.status, await imageResponse.text());
        }
      }
    } catch (imageError) {
