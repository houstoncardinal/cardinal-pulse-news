...
Write a full, original article that expands on this story with analysis, context, and expert perspective. Make it engaging and comprehensive (1000-1500 words). Credit Yahoo Finance as a source. Return ONLY valid JSON, no additional text.`
        ...
        try {
          const imagePrompt = articleData.imagePrompt || 
            `Create a photorealistic, professional business/finance news photograph for: "${articleData.title}". Category: ${mapCategoryToCardinal(category)}. 16:9 aspect ratio, professional lighting, editorial photography style, ultra high resolution, NO text or watermarks.`;
          ...
          if (imageResponse.ok) {
            ...
            if (generatedImageUrl) {
              ...
              const filename = `articles/yahoo-${sanitizedTitle}-${timestamp}.png`;
